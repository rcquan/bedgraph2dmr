...
read_bismark <- function(){
    ...
    tryCatch({setwd("data")}, 
             error = function(e){message("Error: \"data\" directory not found.")})
    
    fileNames <- list.files(pattern = ".bedGraph")
...
subset_by_type <- function(BSmooth_obj, min_cov = 300){
    ...
    type <- str_extract(BSmooth_obj@colData@rownames, "[A-z]+")
    # get user input to classify sample type
    if (length(unique(type)) == 2){
        group1_name <- readline(
            sprintf("Input one to classify %s: case, control, tumor, normal... ", unique(type)[1]))
        group2_name <- readline(
            sprintf("Input one to classify %s: case, control, tumor, normal... ", unique(type)[2]))
    } else{
        stop("Cannot have more than 2 groups. Please check your file IDs.")
    }
    # assign group name to samples
    assign_group_name <- function(X){
        if (X == unique(type)[1]){
            X <- group1_name
        }
        else if (X == unique(type)[2]){
            X <- group2_name
        }
    }
    type <- sapply(type, assign_group_name, USE.NAMES = FALSE)
...
export_tstat_data <- function(BStstat_obj){
    ...
    write.csv(getStats(BStstat_obj), "results/tables/tstat_ttestValues.csv")
}
